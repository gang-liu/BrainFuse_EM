   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_BFL_labelfusion_DEMO_MGH_wrapper.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* BFL_labelfusion_DEMO_MGH_wrapper.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* BFL_labelfusion_DEMO_MGH_wrapper.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* BFL_labelfusion_DEMO_MGH_wrapper (application created by MATLAB Compiler)
		* run_BFL_labelfusion_DEMO_MGH_wrapper.sh (shell script that can be run 
	...
	All Platforms:            
		* BFL_labelfusion_DEMO_MGH_wrapper.so (shared library, file extension 
		  varies by platform)
		* BFL_labelfusion_DEMO_MGH_wrapper.h (library header file)
		* BFL_labelfusion_DEMO_MGH_wrapper.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* BFL_labelfusion_DEMO_MGH_wrapper.ctf (Component Technology File archive)	
	* BFL_labelfusion_DEMO_MGH_wrapper.xml (documentation files)
	* BFL_labelfusion_DEMO_MGH_wrapper.pdb (program database file - if 
	  DEBUG option selected)
	* BFL_labelfusion_DEMO_MGH_wrapper.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* BFL_labelfusion_DEMO_MGH_wrapper.ctf (Component Technology File archive)
	* BFL_labelfusion_DEMO_MGH_wrapper_1_0.dll (component containing 
	  compiled M-code)
	...
	* BFL_labelfusion_DEMO_MGH_wrapper.jar (Java archive containing Java 
	  interface to M-code in BFL_labelfusion_DEMO_MGH_wrapper.ctf)
	* BFL_labelfusion_DEMO_MGH_wrapper.ctf (Component Technology File archive,
	  platform-dependent on target machine)
