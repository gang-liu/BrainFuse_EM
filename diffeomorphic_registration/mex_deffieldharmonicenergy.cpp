...
void deffieldharmonicenergy(int nlhs,
                            mxArray *plhs[],
                            int nrhs,
                            const mxArray *prhs[])
{
   ...
   typename HarmonicEnergyCalculatorType::Pointer
      harmonicEnergyCalculator = HarmonicEnergyCalculatorType::New();

   harmonicEnergyCalculator->SetImage( field );
   harmonicEnergyCalculator->Compute();

   //mexPrintf("done harmonicEnergyCalculator->Compute(); %f sec\n", timer.elapsed());
   //timer.restart();
   
   
   plhs[0] = mxCreateDoubleMatrix(1,1, mxREAL);
   double * outptr = mxGetPr(plhs[0]);
   *outptr = harmonicEnergyCalculator->GetHarmonicEnergy();
                 ...
                 const mxArray *prhs[])
{
   ...
   case 2:
      ...
      {
         case mxSINGLE_CLASS:    
            deffieldharmonicenergy<float,2>(nlhs, plhs, nrhs, prhs);
            break;
         case mxDOUBLE_CLASS:
            deffieldharmonicenergy<double,2>(nlhs, plhs, nrhs, prhs);
            break;
         default:
   ...
   case 3:
      ...
      {
         case mxSINGLE_CLASS:    
            deffieldharmonicenergy<float,3>(nlhs, plhs, nrhs, prhs);
            break;
         case mxDOUBLE_CLASS:
            deffieldharmonicenergy<double,3>(nlhs, plhs, nrhs, prhs);
            break;
         default:
            mexErrMsgTxt("Pixel type unsupported.");
      }
